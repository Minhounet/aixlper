...
The skills below are being designed iteratively, directly with the repo's
author, across many sessions — they are **not** finished/stable the way
the rest of this repo is. Each `SKILL.md` is the source of truth for its
exact current rules; `docs/design-log.md` holds the full session-by-session
history and reasoning behind every rule in them. Don't treat any of them
as complete, and don't remove or "clean up" anything in them or in the
log without the author asking.

- **`skills/igiari-tdd/`** — one-test-per-step TDD workflow (Red / Super
  Green / Refining Refactor). Settled after many iterations; dogfooded via
  kata. Related: `chottomatte-archi` (orthogonal concern), `kaizen-refactor`
  (owns the shared mechanical-syntax checklist).
- **`skills/chottomatte-archi/`** — Clean Architecture / dependency
  inversion rules for Java. Settled after many iterations; dogfooded via
  kata. Related: `igiari-tdd`, `gyakuten-ddd` (DDD strategic patterns).
- **`skills/mujitsu-documentum/`** — Documentum DQL/API scripting
  patterns. **Unverified** — written from general knowledge, not yet run
  against a real docbase; treat every pattern as a draft.
- **`skills/gyakuten-ddd/`** — DDD strategic design patterns (Bounded
  Context, Context Map, etc.), also usable as a learning aid. **New, not
  yet dogfooded.**
- **`skills/kaizen-refactor/`** — two-tier refactoring workflow for
  existing code: Tier 1 IDE-verified mechanical, Tier 2 judgment-call
  gated by trigger thresholds. **New, not yet dogfooded.**
